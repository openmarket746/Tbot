...
def dashboard_loop():
    while True:
        ...
        for sym in top_symbols:
            ...
            ap = s.get("zone_approach", {})
            if (ap.get("at_high") or ap.get("at_low") or
                    ap.get("approaching_high") or ap.get("approaching_low") or
                    ap.get("breached_high") or ap.get("breached_low")):
                zone_alerts.append((sym, s))

        if zone_alerts:
            print(Fore.CYAN + Style.BRIGHT + "  ── ZONE ALERTS ──")
            for sym, s in zone_alerts:
                ap   = s.get("zone_approach", {})
                conf = s.get("zone_confirming", [])
                zh   = s.get("zone_high", 0)
                zl   = s.get("zone_low", 0)
                pr   = s.get("price", 0)
                mom  = ap.get("momentum", "FLAT")

                if ap.get("breached_high"):      zlbl = "BREACHED HIGH"
                elif ap.get("breached_low"):     zlbl = "BREACHED LOW "
                elif ap.get("at_high"):          zlbl = "AT HIGH ZONE "
                elif ap.get("at_low"):           zlbl = "AT LOW ZONE  "
                elif ap.get("approaching_high"): zlbl = "-->HIGH ZONE "
                else:                            zlbl = "-->LOW ZONE  "

                if conf:
                    pnames = " | ".join(p["name"] for p in conf)
                    action_str = f"CONFIRMED: {pnames}"
                    ac = Fore.GREEN if ap.get("at_low") or ap.get("approaching_low") else Fore.RED
                else:
                    action_str = "waiting for reversal pattern..."
                    ac = Fore.YELLOW

                print(ac + Style.BRIGHT +
                      f"  {sym:<8} {zlbl}  price={pr:.5f}({mom})"
                      f"  hi={zh:.5f}  lo={zl:.5f}  => {action_str}")
            print()
...
def analyze(ws, symbol):
    ...
    # ── SIGNAL GENERATION — ZONE-AWARE ENTRY (n.py logic integrated) ──────────
    signal    = "-"
    direction = None
    ...
    # ── PRIMARY: Price AT a zone + trend + reversal pattern confirmation ──────
    ...
    # ── SECONDARY: Approaching zone + tick dominance + displacement ───────────
    elif (approaching_low and is_bull and has_confirm
            and disp_up
            and ten_buy is not None and ten_buy > SIGNAL_THRESHOLD
            and curr_buy > CURR_BUY_THRESHOLD
            and candle_up):
    ...
    elif (approaching_high and is_bear and has_confirm
            and disp_down
            and ten_sell is not None and ten_sell > SIGNAL_THRESHOLD
            and curr_buy < CURR_SELL_THRESHOLD
            and candle_down):
    ...
    # ── FALLBACK: Original tick-dominance + displacement (no zone required) ───
    # Only fires when zone data is unavailable (insufficient candles)
    elif zone_high is None:
        if (is_bull and ten_buy is not None and ten_buy > SIGNAL_THRESHOLD
