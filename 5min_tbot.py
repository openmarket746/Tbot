...
# Persistent across reconnects (keyed by license name)
WIN_MEMORY       = _user_path("win.json")
LOSS_MEMORY      = _user_path("loss.json")
...
DEFAULT_CONFIG = {
    ...
    "pairs": [
        {"symbol": "stpRNG",  "trend": None},
        {"symbol": "stpRNG5", "trend": None},
        {"symbol": "stpRNG4", "trend": None},
        {"symbol": "stpRNG3", "trend": None},
        {"symbol": "stpRNG2", "trend": None}
    ]
}
...
def save_trade_memory(fingerprint, won):
    """Persist fingerprint to win.json / loss.json and update in-memory cache."""
...
def load_memory_on_startup():
    ...
    print(Fore.CYAN + Style.BRIGHT +
          f"[Memory] Loaded {wc} winning & {lc} losing setups from disk.")
...
print(Fore.YELLOW + "\nStake Mode:")
print(Fore.WHITE  + "  [1] Auto Stake  — bot calculates stake from account balance")
print(Fore.WHITE  + "  [2] Manual Stake — you set the daily TP / SL and stake")

while True:
    choice = input(Fore.CYAN + "\nSelect stake mode (1 = Auto / 2 = Manual): ").strip()
    if choice in ("1", "2"):
        STAKE_MODE = "auto" if choice == "1" else "manual"
        break
...
if STAKE_MODE == "manual":
    ...
    while True:
        try:
            STAKE  = float(input(Fore.CYAN + "Enter stake amount per trade        : "))
            MAX_TP = abs(float(input(Fore.CYAN + "Enter daily take-profit target      : ")))
            MAX_SL = abs(float(input(Fore.CYAN + "Enter daily stop-loss limit         : ")))
            MAX_OPEN_TRADES = int(input(Fore.CYAN + "Max open trades per symbol (e.g. 2) : "))
            MAX_SYMBOLS     = int(input(Fore.CYAN + "Max symbols trading at once (e.g. 1): "))
    ...
    print(Fore.GREEN +
          ...
          f"Trades/sym: {MAX_OPEN_TRADES}  |  Symbols: {MAX_SYMBOLS}")
else:
    print(Fore.YELLOW + "\n— Auto Stake Mode —")
    print(Fore.WHITE  + "  Account balance will be fetched after connecting.")
    print(Fore.WHITE  + "  Daily TP  = 10% of balance")
    print(Fore.WHITE  + "  Daily SL  =  5% of balance")
    print(Fore.WHITE  + "  Stake/trade = Daily SL ÷ 4  (4 losses exhaust the daily SL)")
    while True:
        try:
            MAX_OPEN_TRADES = int(input(Fore.CYAN + "Max open trades per symbol (e.g. 2) : "))
            MAX_SYMBOLS     = int(input(Fore.CYAN + "Max symbols trading at once (e.g. 1): "))
            ...
            print(Fore.RED + "Invalid input. Please enter whole numbers.")
    print(Fore.GREEN +
          f"\n✔ Auto — Trades/sym: {MAX_OPEN_TRADES}  |  Symbols: {MAX_SYMBOLS}  "
          f"(TP/SL/Stake will be set after balance is fetched)")
...
def dashboard_loop():
    while True:
        ...
        print(Style.BRIGHT + Fore.CYAN + "=" * 140)
        ...
        print(Fore.WHITE + Style.BRIGHT +
              ...
              f"{'HTF':<7}{'HTF RSI':<9}{'HTF EXH':<9}{'SIGNAL':<8}{'MEM':<8}")
        print(Fore.CYAN + "=" * 140)
...
def analyze(ws, symbol):
    ...
    if total < 5:
        return
    if exhausted or htf_exhausted or trend is None:
        return
    if htf_trend is not None and htf_trend != trend:
        return
...
def on_message(ws, message):
    ...
    if "authorize" in data:
        ...
        with config_lock:
            cfg_pairs = dict(CONFIG_PAIRS_MAP)

        if cfg_pairs:
            for symbol in cfg_pairs.keys():
    ...
    elif "active_symbols" in data and not symbols_loaded:
        symbols_loaded = True
        for s in data["active_symbols"]:
            symbol = s["symbol"]
            if not symbol.startswith("stp"):
                continue
            with config_lock:
                if CONFIG_PAIRS_MAP and symbol not in CONFIG_PAIRS_MAP:
                    continue
        ...
        if not symbol_state[symbol].get("ready"):
            ...
            history_count += 1
            with config_lock:
                expected = len(CONFIG_PAIRS_MAP) if CONFIG_PAIRS_MAP else len(symbols)
            if history_count >= expected:
                ranked = sorted(symbol_state.keys(),
                                key=lambda x: (calculate_5c(x)[0] or 0), reverse=True)
