...
def _send_email(to,subject,html,text):
    ...
    log.info(f"[email] Sending → to={to!r} from={GMAIL_ADDRESS!r} pass_len={len(password_nospace)}")
    ...
    for method, port, use_ssl in [("SSL-465", 465, True), ("TLS-587", 587, False)]:
        try:
            ...
            log.error(f"[email] Auth failed ({method}): {e}")
            last_err = e
        except Exception as e:
            log.info(f"[email] Failed ({method}): {e}")
            last_err = e
...
def submit_proof():
    ...
    def _notify_admin():
        try:
            ...
            log.info(f"[notify] ADMIN_EMAIL={ADMIN_EMAIL!r} GMAIL={GMAIL_ADDRESS!r} PASS_SET={bool(GMAIL_APP_PASS)}")
