...
def _send_email(to,subject,html,text):
    ...
    print(f"[email] Sending → to={to!r} from={GMAIL_ADDRESS!r} pass_len={len(password_nospace)}", flush=True)
    ...
    for method, port, use_ssl in [("SSL-465", 465, True), ("TLS-587", 587, False)]:
        try:
            ...
            server.quit()
            print(f"[email] ✓ Sent via {method} to {to}", flush=True)
            return  # success

        except smtplib.SMTPAuthenticationError as e:
            print(f"[email] Auth failed ({method}): {e}", flush=True)
            last_err = e
        except Exception as e:
            print(f"[email] Failed ({method}): {e}", flush=True)
            last_err = e

    # Both methods failed
    print(f"[email] All methods failed. Last error: {last_err}", flush=True)
...
def submit_proof():
    ...
    def _notify_admin():
        try:
            ...
            print(f"[notify] Admin email sent to {ADMIN_EMAIL!r} for {name!r}", flush=True)
        except Exception as e:
            print(f"[notify] FAILED: {e}", flush=True)
            print(f"[notify] ADMIN_EMAIL={ADMIN_EMAIL!r} GMAIL={GMAIL_ADDRESS!r} PASS_SET={bool(GMAIL_APP_PASS)}", flush=True)
...
def admin_confirm_payment():
    ...
    def _deliver_license():
        try:
            ...
            print(f"[license] Email sent to {email!r}", flush=True)
        except Exception as e:
            print(f"[license] FAILED to email {email!r}: {e}", flush=True)
...
def start_session(data):
    ...
    if existing and existing.get("proc") and existing["proc"].poll() is None:
        ...
        print(f"[session] {name} reconnected to existing bot", flush=True)
        return
    ...
    try:
        ...
        emit("terminal_output", f"\r\n[Bot session started]\r\n")
        print(f"[session] {name} started new bot", flush=True)
    except Exception as e:
        emit("terminal_output", f"\r\n[LAUNCH ERROR] {e}\r\n")
...
def send_command(data):
    ...
    try:
        with open(cmd_file, "w") as f:
            json.dump(data, f)
        print(f"[cmd] wrote {data.get('action','?')} to {cmd_file}", flush=True)
        emit("command_ack", {"ok": True, "action": data.get("action","")})
    except Exception as e:
        print(f"[cmd] failed to write cmd file: {e}", flush=True)
...
def admin_memory_upload():
    ...
    print(f"[memory] {fname} uploaded via admin — {len(data)} entries", flush=True)
...
def admin_resend_license():
    ...
    try:
        ...
        print(f"[resend] License emailed to {email!r} for {name!r}", flush=True)
        flask_session["resend_ok"] = f"License resent to {email}"
    except Exception as e:
        print(f"[resend] FAILED for {email!r}: {e}", flush=True)
...
def admin_db():
    ...
    if request.method == "POST":
        ...
        print(f"[db] license.db uploaded via admin — {len(content):,} bytes", flush=True)
...
print(f"[TBOT] ADMIN_PATH={ADMIN_PATH!r}", flush=True)
print(f"[TBOT] ADMIN_PASSWORD set: {bool(ADMIN_PASSWORD)}", flush=True)
print(f"[TBOT] GMAIL_ADDRESS={GMAIL_ADDRESS!r}", flush=True)
print(f"[TBOT] GMAIL_APP_PASS set: {bool(GMAIL_APP_PASS)}", flush=True)
print(f"[TBOT] ADMIN_EMAIL={ADMIN_EMAIL!r}", flush=True)
print(f"[TBOT] APP_URL={APP_URL!r}", flush=True)
