...
def start_session(data):
    ...
    # Force ANSI colour codes even without a real TTY (Render runs headless)
    env["FORCE_COLOR"]      = "1"
    env["COLORTERM"]        = "truecolor"
    env["TERM"]             = "xterm-256color"

    try:
        # Use a PTY (pseudo-terminal) so the bot process believes it has a
        # real terminal — this makes colorama/isatty() return True and
        # prevents any library from stripping ANSI escape codes.
        try:
            import pty
            ...
            os.close(slave_fd)   # child owns it; parent reads from master_fd
            import io
            pipe = os.fdopen(master_fd, "rb", buffering=0)
        except (ImportError, OSError):
            # PTY unavailable — fall back to plain pipe (colours may need
            # strip=False in colorama to still work)
            log.warning("[session] pty unavailable — falling back to plain pipe")
            proc = subprocess.Popen(
                [python, script],
...
def read_terminal(sid):
    while True:
        ...
        try:
            ...
            # PTY raises OSError (EIO) when the child process closes the slave end
            chunk = b""
        except Exception:
            break
        if not chunk:
