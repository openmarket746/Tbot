...
def _send_email(to,subject,html,text):
    if not GMAIL_ADDRESS or not GMAIL_APP_PASS:
        log.error(f"[email] ERROR: GMAIL_ADDRESS or GMAIL_APP_PASS not set")
    ...
    for method, port, use_ssl in [("SSL-465", 465, True), ("TLS-587", 587, False)]:
        try:
            ...
            server.quit()
            log.info(f"[email] ✓ Sent via {method} to {to}")
            return  # success
    ...
    # Both methods failed
    log.error(f"[email] All methods failed. Last error: {last_err}")
...
log.info(f"[TBOT] ADMIN_PATH={ADMIN_PATH!r}")
