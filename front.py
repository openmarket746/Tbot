import os, sys, pty, json, sqlite3, select, time, errno
...
try:
    import gevent.monkey; gevent.monkey.patch_all(); _async_mode = "gevent"
except ImportError:
...
APP_URL        = os.environ.get("APP_URL",        "https://tbot-07b9.onrender.com")
ADMIN_EMAIL    = os.environ.get("ADMIN_EMAIL",    GMAIL_ADDRESS)
USDT_ADDRESS   = os.environ.get("USDT_ADDRESS",  "0x4391788339806d8d5389abe0898372fc12692b25")
PRICE_USD      = os.environ.get("PRICE_USD",      "10")
...
sessions={}; pids={}; buffers={}; configs={}; names={}
...
def _send_email(to,subject,html,text):
    ...
    with smtplib.SMTP_SSL("smtp.gmail.com",465) as s:
        s.login(GMAIL_ADDRESS,GMAIL_APP_PASS); s.sendmail(GMAIL_ADDRESS,to,msg.as_string())
...
def verify_and_save():
    try:
        ...
        if rq.empty(): return jsonify({"ok":False,"error":"Deriv timed out"}),408
        st,payload=rq.get()
...
def start_session(data):
    ...
    if pid==0:
        ...
        script=os.path.join(BASE_DIR,"Tbot_trend.py"); os.chdir(BASE_DIR)
        python=sys.executable or "python3"
        os.environ["TBOT_USER"]=_safe_name(name); os.environ["TBOT_SID"]=sid
        os.environ["TBOT_CONFIG"]=ucp; os.environ["TBOT_PID_FILE"]=os.path.join(BASE_DIR,f"bot_{sid}.pid")
        try: os.execvp(python,[python,script])
        except Exception as e: os.write(1,f"\r\n[EXEC ERROR] {e}\r\n".encode()); os._exit(1)
    else:
        sessions[sid]=fd; pids[sid]=pid; buffers[sid]=""
...
def read_terminal(sid):
    while True:
        ...
        except (ValueError,OSError): _cleanup_session(sid); break
        if not r: continue
        try: data=os.read(fd,1024).decode(errors="ignore")
        except OSError as e:
            socketio.emit("terminal_output","\r\n[process exited]\r\n" if e.errno in(errno.EIO,errno.EBADF) else f"\r\n[read error:{e}]\r\n",to=sid)
            _cleanup_session(sid); break
        except Exception as e:
            socketio.emit("terminal_output",f"\r\n[error:{e}]\r\n",to=sid); _cleanup_session(sid); break
        if not data: socketio.emit("terminal_output","\r\n[process exited]\r\n",to=sid); _cleanup_session(sid); break
        buffers[sid]=(buffers.get(sid,"")+data)[-20000:]
        socketio.emit("terminal_output",data,to=sid)

def _cleanup_session(sid):
    fd=sessions.pop(sid,None); pid=pids.pop(sid,None)
    buffers.pop(sid,None); configs.pop(sid,None); names.pop(sid,None)
    ucp=os.path.join(BASE_DIR,f"config_{sid}.json")
    if os.path.exists(ucp):
        try: os.remove(ucp)
        except OSError: pass
    if pid is not None:
        try: os.killpg(os.getpgid(pid),_signal.SIGTERM)
        except OSError:
            try: os.kill(pid,_signal.SIGTERM)
            except OSError: pass
        time.sleep(1)
        try: os.killpg(os.getpgid(pid),_signal.SIGKILL)
        except OSError: pass
    bpf=os.path.join(BASE_DIR,f"bot_{sid}.pid")
    if os.path.exists(bpf):
        try:
            with open(bpf) as f: bp=int(f.read().strip())
            try: os.killpg(os.getpgid(bp),_signal.SIGTERM)
            except OSError:
                try: os.kill(bp,_signal.SIGTERM)
                except OSError: pass
...
def terminal_input(data):
    sid=request.sid; key=data["key"]
    if sid in sessions:
        try: os.write(sessions[sid],key.encode())
        except OSError: pass
