...
from flask import (Flask, render_template, request, jsonify,
                   session as flask_session, redirect, Response, Blueprint)
...
# Blueprint registered with url_prefix=ADMIN_PATH so routes are dynamic
...
def _send_email(to,subject,html,text):
    if not GMAIL_ADDRESS or not GMAIL_APP_PASS:
        log.error(f"[email] ERROR: GMAIL_ADDRESS or GMAIL_APP_PASS not set", flush=True)
        raise ValueError("GMAIL_ADDRESS and GMAIL_APP_PASS not configured.")

    # Trim the password — copy-paste from Google sometimes adds whitespace
    password = GMAIL_APP_PASS.strip()
    # Google App Passwords can be stored with or without spaces e.g. "abcd efgh ijkl mnop"
    # Remove all spaces — Google accepts both formats
    password_nospace = password.replace(" ", "")

    log.info(f"[email] Sending → to={to!r} from={GMAIL_ADDRESS!r} pass_len={len(password_nospace)}", flush=True)
    ...
    # Try SSL on port 465 first, then TLS on port 587 as fallback
    for method, port, use_ssl in [("SSL-465", 465, True), ("TLS-587", 587, False)]:
        try:
            if use_ssl:
            ...
            # Try with spaces removed first (most reliable)
            try:
                server.login(GMAIL_ADDRESS, password_nospace)
            except smtplib.SMTPAuthenticationError:
                # Try with original spacing as fallback
                server.login(GMAIL_ADDRESS, password)

            server.sendmail(GMAIL_ADDRESS, to, msg.as_string())
            server.quit()
            log.info(f"[email] ✓ Sent via {method} to {to}", flush=True)
            return  # success

        except smtplib.SMTPAuthenticationError as e:
            log.error(f"[email] Auth failed ({method}): {e}", flush=True)
            last_err = e
        except Exception as e:
            log.info(f"[email] Failed ({method}): {e}", flush=True)
            last_err = e

    # Both methods failed
    log.error(f"[email] All methods failed. Last error: {last_err}", flush=True)
    raise last_err or Exception("Email send failed")
...
def keepalive():
    ...
    for name, bs in list(bot_sessions.items()):
        ...
        if exit_code is not None:
            ...
            log.info(f"[keepalive] {name} bot died (exit={exit_code}) — will restart if user reconnects", flush=True)
        else:
    ...
    log.info(f"[keepalive] {active_count} bots alive, {len(dead)} cleaned", flush=True)
...
def submit_proof():
    ...
    # Send admin notification in background — page renders immediately
    # Email is fire-and-forget; errors go to Render logs only
    def _notify_admin():
        try:
            _email_admin_notify(name, email, proof, pid)
            log.info(f"[notify] Admin email sent to {ADMIN_EMAIL!r} for {name!r}", flush=True)
        except Exception as e:
            log.error(f"[notify] FAILED: {e}", flush=True)
            log.info(f"[notify] ADMIN_EMAIL={ADMIN_EMAIL!r} GMAIL={GMAIL_ADDRESS!r} PASS_SET={bool(GMAIL_APP_PASS)}", flush=True)
...
def show_admin_path():
    ...
    return f"Admin path is: {ADMIN_PATH} | Password set: {bool(ADMIN_PASSWORD)}"
...
def admin_confirm_payment():
    ...
    # Send license email in background — confirmed page renders immediately
    def _deliver_license():
        try:
            _email_license(name, email, tok, rec["expiry"], days)
            log.info(f"[license] Email sent to {email!r}", flush=True)
        except Exception as e:
            log.error(f"[license] FAILED to email {email!r}: {e}", flush=True)
...
def debug_email():
    ...
    # Try connecting to Gmail without sending — just auth check
    password = GMAIL_APP_PASS.strip().replace(" ", "")
    lines.append(f"Password length (no spaces): {len(password)} chars")
    lines.append("")
    ...
    try:
        ...
        server = _s.SMTP_SSL("smtp.gmail.com", 465, timeout=15)
        server.ehlo()
        server.login(GMAIL_ADDRESS, password)
        ...
        lines.append(f"<b style='color:#f85149'>✗ SSL-465 auth FAILED: {e}</b>")
        ssl_ok = False
    except Exception as e:
        lines.append(f"<b style='color:#d29922'>⚠ SSL-465 connection error: {e}</b>")
        ssl_ok = False

    # Test TLS port 587
    lines.append("Testing TLS port 587...")
    try:
        server = smtplib.SMTP("smtp.gmail.com", 587, timeout=15)
        server.ehlo(); server.starttls(); server.ehlo()
        server.login(GMAIL_ADDRESS, password)
        lines.append("<b style='color:#3fb950'>✓ TLS-587 auth OK</b>")
        server.quit()
        tls_ok = True
    except smtplib.SMTPAuthenticationError as e:
        lines.append(f"<b style='color:#f85149'>✗ TLS-587 auth FAILED: {e}</b>")
        tls_ok = False
    except Exception as e:
        lines.append(f"<b style='color:#d29922'>⚠ TLS-587 connection error: {e}</b>")
        tls_ok = False

    lines.append("")

    if not ssl_ok and not tls_ok:
        lines.append("<b style='color:#f85149'>✗ Both methods failed — cannot send email</b>")
        lines.append("")
        ...
        lines.append("  3. Copy the 16-char password exactly — no extra spaces")
        lines.append("  4. Update GMAIL_APP_PASS in Render dashboard → Save → wait for redeploy")
...
def start_session(data):
    ...
    if existing and existing.get("proc") and existing["proc"].poll() is None:
        ...
        log.info(f"[session] {name} reconnected to existing bot", flush=True)
        return
    ...
    try:
        ...
        emit("terminal_output", f"\r\n[Bot session started]\r\n")
        log.info(f"[session] {name} started new bot", flush=True)
    except Exception as e:
        emit("terminal_output", f"\r\n[LAUNCH ERROR] {e}\r\n")
...
def send_command(data):
    ...
    try:
        with open(cmd_file, "w") as f:
            json.dump(data, f)
        log.info(f"[cmd] wrote {data.get('action','?')} to {cmd_file}", flush=True)
        emit("command_ack", {"ok": True, "action": data.get("action","")})
    except Exception as e:
        log.error(f"[cmd] failed to write cmd file: {e}", flush=True)
...
@admin_bp.route("/memory/download")
@_admin_required
def admin_memory_download():
    ...
    if not os.path.exists(fpath):
        ...
        return redirect(ADMIN_PATH + "/memory")
...
@admin_bp.route("/memory/upload", methods=["POST"])
@_admin_required
def admin_memory_upload():
    ...
    log.info(f"[memory] {fname} uploaded via admin — {len(data)} entries", flush=True)
    flask_session["mem_flash"] = f"✓ {fname} uploaded successfully — {len(data)} setups loaded."
    return redirect(ADMIN_PATH + "/memory")


@admin_bp.route("/resend-license", methods=["POST"])
@_admin_required
def admin_resend_license():
    ...
    try:
        ...
        log.info(f"[resend] License emailed to {email!r} for {name!r}", flush=True)
        flask_session["resend_ok"] = f"License resent to {email}"
    except Exception as e:
        log.error(f"[resend] FAILED for {email!r}: {e}", flush=True)
...
def admin_db():
    ...
    if request.method == "POST":
        ...
        log.info(f"[db] license.db uploaded via admin — {len(content):,} bytes", flush=True)
...
# Register admin blueprint with the secret path prefix
app.register_blueprint(admin_bp, url_prefix=ADMIN_PATH)

# Startup confirmation — visible in Render logs
log.info(f"[TBOT] Starting — async_mode={_async_mode} BASE_DIR={BASE_DIR}", flush=True)
log.info(f"[TBOT] ADMIN_PATH={ADMIN_PATH!r}", flush=True)
log.info(f"[TBOT] ADMIN_PASSWORD set: {bool(ADMIN_PASSWORD)}", flush=True)
log.info(f"[TBOT] GMAIL_ADDRESS={GMAIL_ADDRESS!r}", flush=True)
log.info(f"[TBOT] GMAIL_APP_PASS set: {bool(GMAIL_APP_PASS)}", flush=True)
log.info(f"[TBOT] ADMIN_EMAIL={ADMIN_EMAIL!r}", flush=True)
log.info(f"[TBOT] APP_URL={APP_URL!r}", flush=True)
