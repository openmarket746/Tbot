...
def _memory_summary(setup_key):
    """Return (wins, losses, last_outcome) for a setup key.
    last_outcome is derived from whichever memory list has more recent records
    by comparing list lengths (records are appended in order).
    """
    wl = S["win_memory"].get(setup_key, [])
    ll = S["loss_memory"].get(setup_key, [])
    w, l = len(wl), len(ll)
    # Most recent outcome: compare last index in each list
    last = None
    if w and l:
        last = "win" if w >= l else "loss"   # whichever grew more recently wins tie
    elif w:
        last = "win"
    elif l:
        last = "loss"
...
async def _on_message(msg):
    ...
    if t == "authorize":
        ...
        if contract_id:
            ...
            log(f"Contract #{contract_id} {trade_type} {sym} open", "ok")
        else:
...
def _open_count_for_sym(sym):
    """Count currently open contracts for a given symbol."""
    return sum(1 for info in S["open_contracts"].values() if info.get("sym") == sym)

def _syms_with_open_trades():
    """Return the set of symbols that currently have at least one open contract."""
    return {info.get("sym") for info in S["open_contracts"].values() if info.get("sym")}
...
async def check_signal(sym, a):
    ...
    # ── Trade memory ──────────────────────────────────────────────────────────
    w, l, last_outcome = _memory_summary(setup_key)
    mem_note = ""
    if w + l > 0:
        wr_pct   = int(w / (w + l) * 100)
        mem_note = f"mem={w}W/{l}L({wr_pct}%) last={last_outcome}"
        log_file("memory", f"{sym} {setup_key}: {mem_note}")
        if l > w and (w + l) >= 3:
            log(f"⚠ {sym} {sig}·{setup} hist={w}W/{l}L — proceeding", "warn")
    ...
    # ── Fire one trade per available slot (up to max_trades_per_sym) ─────────
    for slot in range(slots_open):
        # Build a unique dedup key per slot so each trade is tracked separately
...
# ── Web-mode print logger (replaces the PTY draw loop) ───────────────────────
def log_print(msg, kind="info"):
    """Print a coloured log line to stdout (captured by front.py → xterm)."""
    ts = datetime.now().strftime("%H:%M:%S")
    c = {"info":WHT,"ok":GRN,"error":RED,"trade":CYN,
         "win":GRN,"loss":RED,"signal":YLW,"warn":ORG}.get(kind, WHT)
    i = {"info":"·","ok":"✓","error":"✗","trade":"◆",
    ...
    # Print to stdout — front.py streams this to the xterm terminal
    sys.stdout.write(line + "\r\n")
    sys.stdout.flush()
    log_file(kind, msg)

# Monkey-patch log → log_print so all existing log() calls go to stdout
log = log_print

# ── Status ticker (prints a summary line every 30 s so terminal is not silent) ─
async def status_ticker():
    while True:
        await asyncio.sleep(30)
        if not S["running"]: continue
        conn_s  = "LIVE"   if S["connected"]  else "OFFLINE"
        auth_s  = "AUTH"   if S["authorized"] else "PENDING"
        run_s   = "RUNNING" if S["running"]   else "STOPPED"
        bal_s   = f"${S['balance']:.2f} {S['currency']}"
        pnl_c   = "+" if S["pnl"] >= 0 else ""
        pnl_s   = f"{pnl_c}${S['pnl']:.2f}"
        tp_s    = f"TP:${S['session_tp']:.2f}" if S["session_tp"] > 0 else "TP:OFF"
        sl_s    = f"SL:${S['session_sl']:.2f}" if S["session_sl"] > 0 else "SL:OFF"
        sk_s    = f"Stake:${S['default_stake']:.2f}" if S["default_stake"] > 0 else ""
        open_n  = len(S["open_contracts"])
        syms_n  = len(_syms_with_open_trades())
        log(f"{conn_s}|{auth_s}|{run_s}  Bal:{bal_s}  PnL:{pnl_s}  W:{S['wins']} L:{S['losses']}  Open:{open_n}  ActiveSyms:{syms_n}/{S['max_syms_trading']}  {tp_s}  {sl_s}  {sk_s}", "info")
